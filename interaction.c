...
void coagulation(double dt0){
	...
	double a_pb1,a_pb11,a_pb2,a_pb22,a_pb3,vr0,v1,v2,delta_v,AREA,dr,a_max,dt_new,dt1,sub_t;
	...
	for (i=ring_num-1;i>=2;i--){
		...
		for(j=peb_size_num-1;j>=1;j--){
			...
			if(coag_kernel(a_pb1,a_pb2,delta_v,peb_map[i].rho[j],peb_map[i].rho[jj],dt1,peb_map[i].rad_med)>=frac_s){
			frac_s=coag_kernel(a_pb1,a_pb2,delta_v,peb_map[i].rho[j],peb_map[i].rho[jj],dt1,peb_map[i].rad_med);
			}
	...
	if(dt1 < dt0) printf("ring_num=%d\tdt=%f\t%f\tmax_frac_s=%f\n",i,dt1,dt0,frac_s);
	sub_t=0.0;
	while(sub_t<dt0){
		for(j=peb_size_num-1;j>=1;j--){
                        if(peb_map[i].surf_dens[j] < 1e-5) continue;
