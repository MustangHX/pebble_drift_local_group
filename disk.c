...
return pow(3.0,0.2)*pow(2.0,-1.4)*pow(M_PI,-0.4)\
	*pow(mu*m_p/gamma0/k_B,0.2)*pow(opa/sig_sb,0.2)*pow(alpha,-0.2)*pow(G*m_star,0.3)*pow((1-sqrt(r_star*LUNIT/r))*mdot,0.4)*pow(r,-0.9);
}
...
return pow(3.0,-1.3)*pow(2.0,1.6)*pow(M_PI,-0.9)\
	  ...
	  *pow(G*m_star,0.55)*pow((1-sqrt(r_star*LUNIT/r))*mdot,0.4)*pow(r,-1.65);
}
